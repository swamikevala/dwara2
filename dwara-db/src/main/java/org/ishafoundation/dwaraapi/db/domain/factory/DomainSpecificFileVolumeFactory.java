...
public class DomainSpecificFileVolumeFactory {
    ...
    private static final Map<String, Class<? extends FileVolume>> instances = new HashMap<>();

    public static void register(String domainSpecificEntity, Class<? extends FileVolume> instance) {
    ...
    public static FileVolume getInstance(Domain domain, int fileId, Volume volume) {
        ...
        if (instances.containsKey(domainSpecificFileVolumeTableName)) {
        	...
        	try {
				...
				entity = entityAsClass.getDeclaredConstructor(cArg).newInstance(fileId, volume);
			} catch (Exception e) {
				// swallow it...
				e.printStackTrace();
			}
