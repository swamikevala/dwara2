...
public class ClipIdsGetter extends CatDVInteractor {
	...
	public List<Integer> getClipIds(String jsessionId, int catalogId) throws Exception {
    	String endpointUrl = getURLPrefix() + "/catdv/api/8/catalogs?catalogID=" + catalogId + "&include=userfields";
		...
		try {
			...
			List<Map<String, Object>> clips = JsonPathUtil.getArray(response , "data.items");
