...
@Component
public class JobServiceRequeueHelper extends UserRequestHelper{
	...
	public Job requeueJob(int jobId, String userName) throws Exception{
		...
		try {		
	    	...
	    	userRequest = createUserRequest(Action.requeue, userName, data);
...
//	    	logger.info(DwaraConstants.USER_REQUEST + userRequestId);
			...
			Request jobUserRequest = jobToBeRequeued.getRequest().getRequestRef();
			jobUserRequest.setStatus(Status.queued);
