...
//						}
			...
			if(rewriteSystemRequestList.size() > 0) { // if there are any rewrite request pending, dont add all its jobs to the queue
				...
				jobList.addAll(jobDao.findTop2ByStoragetaskActionIdIsNotNullAndRequestActionIdAndStatusOrderByRequestId(Action.rewrite, Status.queued)); // add only one rewrite specific job to the collection
			}
			else
