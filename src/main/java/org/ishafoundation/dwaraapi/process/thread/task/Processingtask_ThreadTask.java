...
@Component
@Scope("prototype")
public class Processingtask_ThreadTask implements Runnable{


	private static final Logger logger = LoggerFactory.getLogger(Processingtask_ThreadTask.class);
	...
	public void run(){
		...
		try {
			...
			if(job.getStatus() == Status.cancelled || job.getStatus() == Status.aborted) { 	
				...
				Processingtask task = processingtaskDao.findById(taskId).get();
				taskName = task.getName();
				logger.trace(job.getId() + " " + taskName);
				
				ITaskExecutor taskExecutor = TaskFactory.getInstance(applicationContext, taskName);
				
				String inputLibraryName = inputLibrary.getName();
				Libraryclass libraryclass = inputLibrary.getLibraryclass();
				int libraryclassId = libraryclass.getId();
				String libraryCategory = libraryclass.getCategory();
				...
				if(isComplete) {
					...
					if(outputLibraryName != null) {
						...
						if(outputLibrary == null) {// not already created.
						    ...
						    outputLibrary.setLibraryRef(inputLibrary); //sourceLibraryId
						    outputLibrary.setFileCount(1111111);
						    outputLibrary.setFileStructureMd5("not needed");
						    Ingestconfig ic = ingestconfigDao.findByTaskIdAndTasktypeAndInputLibraryclassIdAndTapesetId(taskId, Tasktype.processing, libraryclassId, 0);
						    Libraryclass outputLibraryclass = ic.getOutputLibraryclass();
