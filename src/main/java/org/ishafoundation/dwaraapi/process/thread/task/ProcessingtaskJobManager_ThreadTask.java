...
@Component
@Scope("prototype")
public class ProcessingtaskJobManager_ThreadTask implements Runnable{

	private static final Logger logger = LoggerFactory.getLogger(ProcessingtaskJobManager_ThreadTask.class);
    ...
    public void run() {
		...
		Library outputLibrary = job.getOutputLibrary();
		
		int taskId = job.getTaskId();
		Processingtask task = processingtaskDao.findById(taskId).get();

		if(outputLibrary != null) {
			Libraryclass outputLibraryclass = outputLibrary.getLibraryclass();
		...
		Collection<LogicalFile> selectedFileList = getLogicalFileList(task.getFiletype(), inputLibraryPath);
...
//				continue;			
			...
			Processingtask_ThreadTask tasktype = applicationContext.getBean(Processingtask_ThreadTask.class);
			tasktype.setJob(job);
	...
	private Collection<LogicalFile> getLogicalFileList(Filetype filetype, String inputLibraryPath){
