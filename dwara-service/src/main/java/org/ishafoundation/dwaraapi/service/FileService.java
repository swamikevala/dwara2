...
public class FileService extends DwaraService{
	 ...
	 * select file1.id, artifact.name from artifact join file1 on artifact.id = file1.artifact_id where artifact.name like '%Sadhguru%Instagram%02-Apr-2021%' and artifact.artifactclass_id not like '%proxy-low' and file1.pathname = artifact.name;
	 * when the order of the searchstr is expected to be haphazard
	 * select file1.id, artifact.name from artifact join file1 on artifact.id = file1.artifact_id where artifact.name like '%02-Apr-2021%' and artifact.name like '%Sadhguru%' and artifact.artifactclass_id not like '%proxy-low' and file1.pathname = artifact.name;
	 * 
	 * subqueries take longer when the searchstr is complicated
	 * select id, pathname from file1 where pathname in (select name from artifact where name like '%Sadhguru%Instagram%02-Apr-2021%' and artifactclass_id not like '%proxy-low');
	 * 
...
//    	logger.info(DwaraConstants.USER_REQUEST + userRequestId);
    ...
    private void validate(List<Integer> fileIds, Map<Integer, org.ishafoundation.dwaraapi.db.model.transactional.File> fileId_FileObj_Map) {
    	...
    	for (Integer nthFileId : fileIds) {
			 ...
			 * domainUtil.getDomainSpecificFile(nthDomain, nthFileId); if(fileFromDB !=
			 * null) { fileId_FileObj_Map.put(nthFileId, fileFromDB);
			 * fileId_Domain_Map.put(nthFileId, nthDomain); break; } }
			 */
    ...
    public void deleteFile(int fileId){
		 ...
		 * org.ishafoundation.dwaraapi.db.model.transactional.domain.File fileFromDB =
		 * domainUtil.getDomainSpecificFile(nthDomain, fileId); if(fileFromDB != null) {
		 * fileFromDB.setDeleted(true);
		 * FileRepository<org.ishafoundation.dwaraapi.db.model.transactional.domain.
		 * File> domainSpecificFileRepository =
		 * domainUtil.getDomainSpecificFileRepository(nthDomain);
		 * domainSpecificFileRepository.save(fileFromDB); break; } }
		 */
    }
}
